...
# Define weights (input → hidden, hidden → output)
W1 = np.array([[0.5, 1.2, -1.0, 0.0], [-0.4, 0.8, 0.3, 0.6], [0.1, -0.6, 0.5, 1.0]])  # shape: (3, 4)
W2 = np.array([[1.0, -0.6, 0.8, 0.2], [-0.3, 0.9, -1.0, 0.5]])  # shape: (4, 2)
...
def get_edge_traces(x_vals):
    ...
    # Build node colors
    act = {'hidden': h / h.max() if h.max() != 0 else h, 'output': y / np.abs(y).max() if np.abs(y).max() != 0 else y}
...
# Plotly FigureWidget for dynamic updates
fig = go.FigureWidget()
edge_trace = go.Scatter(x=[], y=[], mode='lines', line=dict(width=3), hoverinfo='none')
node_trace = go.Scatter(x=node_x, y=node_y, mode='markers+text', marker=dict(size=30, color=[]), text=node_labels,
