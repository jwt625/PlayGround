...
<head>
    ...
    <style>
        ...
        .link:hover { stroke: #333; stroke-opacity: 1; stroke-width: 3px; }
...
<body>
    <svg id="graph"></svg>
    <div id="tooltip"></div>
    <div class="zoom-hint">Mouse wheel: zoom | Drag background: pan | Drag nodes: reposition</div>
    <div id="controls">
        ...
        <div class="control-group">
            ...
            <div class="info-text">Shows the N most connected nodes</div>
        </div>
        <div class="control-group">
        ...
        <div class="stats">
            <div><strong>Visible:</strong> <span id="visible-count">0</span> nodes, <span id="visible-edges">0</span> edges</div>
    ...
    <script>
        ...
        function resetFilters() {
            document.getElementById('top-n-slider').value = initialNodes;
            document.getElementById('top-n-value').textContent = initialNodes;
            filterGraph();
        }
        ...
        function filterGraph() {
            ...
            const currentNodes = allNodes.slice(0, Math.min(topN, allNodes.length));

            // Get node IDs for edge filtering
            const nodeIds = new Set(currentNodes.map(n => n.id));

            // Filter edges - only include edges where BOTH nodes are visible
            // Handle both string IDs (initial) and object references (after simulation)
            ...
            document.getElementById('visible-edges').textContent = currentEdges.length;
        ...
        function updateGraph(nodes, edges) {
            ...
            // Update labels (only for top 50 nodes by degree)
            const topLabelNodes = nodes.slice(0, 50);
            const label = labelGroup.selectAll("text")
                .data(topLabelNodes, d => d.id);
        ...
        function showNodeTooltip(event, d) {
            ...
            tooltip.innerHTML = `<strong>${d.label}</strong><br>Type: ${d.type}<br>Degree: ${d.degree}<br><br>${d.description}`;
        ...
        function showEdgeTooltip(event, d) {
            ...
            tooltip.innerHTML = `<strong>${sourceName} → ${targetName}</strong><br>Weight: ${d.weight.toFixed(1)}<br><br>${d.description}`;
            tooltip.style.display = 'block';
            tooltip.style.left = (event.pageX + 10) + 'px';
            tooltip.style.top = (event.pageY + 10) + 'px';
        }
...
print(f"Opens with top {args.initial_nodes} nodes")
print(f"Features:")
print(f"  - Mouse wheel zoom, drag to pan")
print(f"  - Hover on nodes AND edges for tooltips")
print(f"  - Top N filter: shows the N most connected nodes")
