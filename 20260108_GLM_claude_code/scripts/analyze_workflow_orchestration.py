    ...
    for idx, entry in enumerate(entries):
        ...
        if resp_tool_uses:
            ...
            for tool in resp_tool_uses:
                ...
                if tool_name == 'Task':
                    subagent_type = tool.get('input', {}).get('subagent_type')
                    if subagent_type:
                        ...
                        stats['task_spawns'].append({
                            ...
                            'description': tool.get('input', {}).get('description', '')[:80]
                        })
...
if __name__ == '__main__':
    ...
    for spawn in stats['task_spawns'][:10]:
        print(f"Entry {spawn['entry_idx']:3d} | {spawn['timestamp']} | {spawn['subagent_type']:15s}")
