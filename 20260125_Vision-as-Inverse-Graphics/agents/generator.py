...
from utils.common import get_image_base64, get_model_response, tournament_select_best, set_image_max_size
...
class GeneratorAgent:
    ...
    def __init__(self, args: Dict[str, Any], verifier: VerifierAgent) -> None:
        ...
        # Add Qwen-specific parameters to prevent repetition and control output
        if 'qwen' in self.config.get("model", "").lower():
            self.init_chat_args['temperature'] = 0.3
            # Qwen3-VL needs more tokens for verbose code generation
            if 'qwen3' in self.config.get("model", "").lower():
                self.init_chat_args['max_tokens'] = 8000
            else:
                self.init_chat_args['max_tokens'] = 2000
            # Use smaller images for Qwen models due to context length limits
            set_image_max_size(512)
