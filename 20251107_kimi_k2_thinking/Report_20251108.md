 Server Information
  ...
  - GET /v1/models - Lists available models ✅ (requires auth)
  ...
  - POST /tokenize - Convert text to tokens ✅ (no auth required)
    - Example: "Hello, how are you?" → 6 tokens
  - POST /detokenize - Convert tokens to text ✅ (no auth required)

  Anthropic-Compatible API

  - POST /v1/messages - Anthropic Messages API ✅ (requires auth)
  ...
  - POST /invocations - SageMaker endpoint ✅ (routes to completions)

  ---
  ❌ Unsupported Endpoints (Model Limitations)
  ...
  - Format: Authorization: Bearer sk-fc95e593c28987dd59177edaaee673f4f2a869f5a3f2d5cd7d9f6f7cef609f5b
  ...
  - ✅ Reasoning content (thinking model feature)
  ...
  tasks!
   ...
   ┌──────────────────────────────────────────────────────────────────────────────┐
   │ ENDPOINTS REQUIRING AUTHENTICATION (Bearer Token)                            │
   └──────────────────────────────────────────────────────────────────────────────┘
   ✓ /v1/models                    - List available models
   ✓ /v1/chat/completions         - OpenAI-compatible chat completions
   ✓ /v1/completions              - OpenAI-compatible text completions
   ✓ /v1/messages                 - Anthropic-compatible messages API
   ✓ /v1/responses                - Anthropic-compatible responses API
   ✓ /v1/embeddings               - Embeddings API (model doesn't support)

   ┌──────────────────────────────────────────────────────────────────────────────┐
   │ ENDPOINTS NOT REQUIRING AUTHENTICATION                                       │
   └──────────────────────────────────────────────────────────────────────────────┘
   ✓ /health                       - Health check endpoint
   ✓ /ping                         - SageMaker ping endpoint
   ✓ /load                         - Server load metrics
   ✓ /version                      - API version information
   ✓ /metrics                      - Prometheus metrics (detailed vLLM stats)
   ✓ /tokenize                     - Convert text to token IDs
   ✓ /detokenize                   - Convert token IDs to text
   ✓ /invocations                  - SageMaker invocations endpoint (!)
   ...
   Key Observations:
     • Very consistent throughput across different response lengths
     • Slight overhead for very short completions (<20 tokens)
     • Long context (1000+ tokens) doesn't significantly impact throughput
     • Time to first token: ~1-2 seconds (from metrics)
     • Model uses reasoning_content field (thinking model)

   Model Specifications:
     • Model: kimi-k2-thinking
     • Max context: 12,288 tokens
     • Type: Reasoning/thinking model
     • Backend: vLLM 0.11.1rc6
   ...
   For PRODUCTION use:
     • Use authenticated endpoints (/v1/chat/completions, /v1/messages)
     • Expect ~10.5 tokens/second generation speed
     • Use /tokenize to pre-calculate token counts (no auth needed)
     • Monitor with /metrics endpoint (no auth needed)
     • Use streaming for better user experience on long responses

   For TESTING/MONITORING:
     • Use /invocations (no auth) for quick tests
     • Use /health, /ping for uptime monitoring
     • Use /load for capacity planning
