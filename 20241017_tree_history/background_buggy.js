...
function initializeEventListeners() {
  ...
  // Message listener in initializeEventListeners()
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  // For toggleTracking, we need to handle it as an async operation
  if (request.action === "toggleTracking") {
    ...
    chrome.storage.local.set({ isTracking: isTracking })
      .then(() => {
        if (isTracking) {
          // When starting tracking, capture all existing tabs
      ...
      .catch(error => {
        ...
        isTracking = !isTracking; // Revert state on error
        updateIcon(isTracking);
  ...
  // For all other messages, use the handleOtherMessages function
